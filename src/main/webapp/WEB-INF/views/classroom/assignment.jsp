...
<html lang="en">
  ...
  function makeAssignment(assignmentInfo){
	...
	let littleTitle =  createDiv("littleTitle", "littleTitle" ,null, "과제");
	 ...
	 claAssignment.appendChild(resAssbigList);
		...
		let resAssName = createDiv("resAssName["+i+"]", null , null ,resAssignmentInfo[i].assName);
  ...
  function moveAssignmentDetail(assignmentInfo ,idx){
	  ...
	  claAssignmentDetail.appendChild(createDiv("assName" , null ,null, "과제 제목 : " + assignmentInfo[idx].assName));
	  claAssignmentDetail.appendChild(createDiv("assInfo" , null ,null,"과제 설명 : " + assignmentInfo[idx].assInfo));
	  claAssignmentDetail.appendChild(createDiv("assStartDate" , null ,null,"과제 시작 날짜 : " +assignmentInfo[idx].assStartDate.substr(0,10)));
	  claAssignmentDetail.appendChild(createDiv("assEndDate" , null ,null,"과제 마감 날짜 : " +assignmentInfo[idx].assEndDate.substr(0,10)));
	  claAssignmentDetail.appendChild(createDiv("assMaxGrade" , null ,null,"과제 만점 : " +assignmentInfo[idx].assMaxGrade));
	  
	  let uploadInfo = createTextArea("subInfo", "3","83",  "과제 내용을 입력하세요 * 필수 *");
	  ...
	  claAssignmentDetail.appendChild(uploadInfo);
	  ...
	  claAssignmentDetail.appendChild(createDiv("assSubmittedFileList" , null ,null, "과제 파일"));
	  ...
	  for(i=0 ; i < assignmentInfo[idx].submittedAssignment.length; i++ ){
		  ...
		  submittedFileName[i] = createDiv("assSubmittedFileName["+i+"]" , null ,null, assignmentInfo[idx].submittedAssignment[i].sbfFilName )
	  	  ...
	  	  claAssignmentDetail.appendChild(submittedFileList);
	  }
