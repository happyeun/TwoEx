...
function makeManageCurriculum(curriculumInfo){
	  ...
	  let curList =[];
		...
		for( i = 0 ; i < curriculumInfo.length; i++){
			...
			curList[i].appendChild(createDiv("mCurName["+i+"]", null , null ,curName));
			curList[i].appendChild(createDiv("mCurInfo["+i+"]", null, null, curInfo));
			curList[i].appendChild(createDiv("mCurStartDate["+i+"]", null, null, curStartDate));
			curList[i].appendChild(createDiv("mCurEndDate["+i+"]", null, null, curEndDate));
			curList[i].appendChild(createDiv("mCurTime["+i+"]", null, null, curTime));
		...
		curInfoDiv.appendChild(createInput(null, "curInfo", "커리큘럼내용", null));
...
function delCurriculum(curCode){
	  clientData = ""; //초기화 하지않으면 쌓인다.
		...
		+ "&curCode=" + document.getElementsByName("curCode")[0].value
	  alert(clientData);
	  alert("정말 삭제하시겠습니까?");// yes or no 구성해야함
				...
				<div class="common__menu">
					...
					<div class="common__menu__list">
						...
						<div class="common__menu__item parentItem"
							...
							관리</div>
						<div class="common__menu__item parentItem"
							onClick="moveManageGrade(claSelCode, claCteCode, claPrdCode)">성적
							관리</div>
						<div class="common__menu__item parentItem"
							onClick="moveManageNotice(claSelCode, claCteCode, claPrdCode)">공지
							관리</div>
						<div id="moveDashboard" class="common__menu__item parentItem2"
