...
<script>
	...
	function addAssignmentCallBack(ajaxData) {
		...
		if (list[0].assCode != "special" || list != '') {
			...
			no.setAttribute("class", "content__btn btn__micro");
	...
	function activateUpd(div) {
		this.tempDiv = div;
		let productList__zone1 = document.getElementsByClassName("createZone")[1];
		let assName = document.getElementsByName("assName")[0];
		let assInfo = document.getElementsByName("assInfo")[0];
		let assStartDate = document.getElementsByName("assStartDate")[0];
		let assEndDate = document.getElementsByName("assEndDate")[0];
		...
		if (!isMagic) {
			...
			productList__zone1.appendChild(cancel);
			isMagic = true;
		}
		assName.focus();
	}
	...
	function activateDel(div) {
		...
		yes.setAttribute("class", "content__btn btn__micro");
	...
	function makeAssignment(classroomInfo) {
		...
		for (i = 0; i < classroomInfo[0].assignment.length; i++) {
			...
			let upd = createDiv(null, 'content__btn btn__micro', null, '수정');
			upd.addEventListener("click", function() {
				activateUpd(div);
			});
			let del = createDiv(null, 'content__btn btn__micro', null, '삭제');
			del.addEventListener("click", function() {
				activateDel(div);
			});
			let detail = createDiv(null, 'content__btn btn__micro', null, '상세');
		...
		<div id="common__wrapper">
			...
			<div class="common__content__zone">
				...
				<div class="createZone">
					...
					<div>
						<label>내용</label><br>
						<textarea name="assInfo" rows="10" cols="28"></textarea>
					</div>
					<div>
						<label>시작일</label><br> <input type="date" name="assStartDate">
					</div>
					<div>
						<label>마감일</label><br> <input type="date" name="assEndDate">
					</div>
					<div>
						<label>최대점수</label><br> <input type="text" name="assMaxGrade"
							placeholder="최대점수" value="100">
					</div>
