...
<script>
	...
	function makeStudent(classroomInfo) {
		...
		for (i = 0; i < classroomInfo[0].student.length; i++) {
			studentDiv[i] = createDiv("student" + i, "product__item", null,
					null);
			let box = [];
			...
			box.push(createDiv("stuBuyNickname", null, null,
			...
			box
					.push(createDiv(
							...
							classroomInfo[0].student[i].stuBuyGender == 'M' ? '남'
									: '여'));
			box.push(createDiv("stuBuyAge", null, null,
					(year - classroomInfo[0].student[i].stuBuyAge)+1 + '세'));
			box.push(createDiv("stuBuyRegion", null, null,
					classroomInfo[0].student[i].stuBuyRegion));
			box.push(createDiv("stuBuyEmail", null, null,
